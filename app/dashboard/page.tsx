...
import { Flame, Clock, DollarSign, Microscope, Briefcase } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const scored = useMemo(
    () =>
      opportunities
        .map((o) => ({ opp: o, match: scoreOpportunity(o, profile) }))
  ...
  const totalAward = scored.reduce((sum, r) => sum + (r.opp.award ?? 0), 0);
  const researchCount = scored.filter((r) => r.opp.category === "RESEARCH").length;
  const internshipCount = scored.filter((r) => r.opp.category === "INTERNSHIP").length;
  ...
  if (loaded && !profile.grade) {
    return (
      <div className="mx-auto max-w-xl px-4 py-20 text-center sm:px-6">
        <h1 className="font-display text-2xl font-semibold">Finish your profile first</h1>
        <p className="mt-2 text-ink-soft">
          Add your grade and interests so SCOUT can calculate personalized matches.
        </p>
        <Link href="/profile">
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-6 rounded-xl border border-line bg-white p-6">
        <p className="font-display text-lg font-semibold">
          {matchCount} opportunit{matchCount === 1 ? "y" : "ies"} match your profile
        </p>
        <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm text-ink-soft">
          <Stat icon={Flame} label={`${scored.filter((r) => r.match.tier === "STRONG").length} strong matches`} />
          <Stat icon={Clock} label={`${upcomingDeadlines.length} deadlines coming up`} />
          <Stat icon={DollarSign} label={`$${totalAward.toLocaleString()} in scholarship opportunities`} />
          <Stat icon={Microscope} label={`${researchCount} research opportunities`} />
          <Stat icon={Briefcase} label={`${internshipCount} internships`} />
        </div>
      </div>
      ...
      <div className="mt-10">
        <h2 className="font-display text-lg font-semibold">Deadlines coming up</h2>
        <div className="mt-4 divide-y divide-line rounded-xl border border-line bg-white">
          {upcomingDeadlines.map((o) => {
            const days =
              now != null
                ? Math.ceil((new Date(o.deadline!).getTime() - now) / (1000 * 60 * 60 * 24))
                : null;
            return (
              <Link
                key={o.id}
              ...
              >
                ...
                <span className="text-sm text-muted">
                  {days == null ? "…" : days > 0 ? `${days} days` : "Deadline passed"}
                </span>
              </Link>
            );
          })}
